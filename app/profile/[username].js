...
export default function PublicProfileScreen() {
    ...
    <View style={styles.scrollableHeaderContainer}>
      ...
      <View style={styles.localSearchContainer}>
        ...
        <TextInput
          ...
          placeholder={i18n.t('public_collection.search', { name: username })}
  ...
  return (
    <View style={styles.container}>
      {/* 1. EN-TÊTE FIXE : Bouton Retour et Titre */}
      <View style={styles.header}>
        <TouchableOpacity style={styles.backButton} onPress={() => router.back()}>
          <MaterialIcons name="arrow-back-ios" size={22} color="#fff" style={{ marginLeft: 6 }} />
        </TouchableOpacity>
        <Text style={styles.headerTitle}>
          {i18n.t('public_collection.title', { name: username })}
        </Text>
      </View>

      {/* 2. LISTE DÉFILANTE AVEC LE HEADER INJECTÉ */}
      <FlatList
        style={{ flex: 1 }} // Indispensable pour que le scroll de la liste fonctionne correctement sous un en-tête fixe
        data={displayedGames}
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>{i18n.t('public_collection.empty', { name: username })}</Text>}
      />
    </View>
  );
}
...
const styles = StyleSheet.create({
  ...
  listContainer: { paddingHorizontal: 16, paddingBottom: 40, flexGrow: 1 }, // flexGrow ajouté
