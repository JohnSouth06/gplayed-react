...
const UserCard = memo(({ item, followingIds, handleToggleFollow, router }) => {
    ...
    // CORRECTION ICI : On utilise item.avatar_url et non item.avatar
    let finalAvatarUrl = null;
    if (item.avatar_url && typeof item.avatar_url === 'string' && item.avatar_url.trim() !== '' && item.avatar_url !== 'null') {
      let cleanPath = item.avatar_url.trim();
      
      // Suppression des chemins relatifs de l'API (ex: "../images/default.png" -> "images/default.png")
      while (cleanPath.startsWith('../') || cleanPath.startsWith('./')) {
        cleanPath = cleanPath.replace(/^\.\.?\//, '');
      }
      
      // Si c'est une URL absolue (Google, Discord, etc.)
      if (cleanPath.startsWith('http')) {
        finalAvatarUrl = cleanPath;
      } 
      // Si ça commence par //
      else if (cleanPath.startsWith('//')) {
        finalAvatarUrl = `https:${cleanPath}`;
      } 
      // Sinon, c'est une image locale (ex: "uploads/avatars/image.jpg")
      else {
        const prefix = cleanPath.startsWith('/') ? '' : '/';
        finalAvatarUrl = `https://www.g-played.com${prefix}${cleanPath}`;
      }
    }
        ...
        <View style={styles.avatarContainer}>
          {finalAvatarUrl && !imgError ? (
            <ExpoImage 
              ...
              onError={() => setImgError(true)} // Bascule sur la lettre si l'image n'est pas trouvée (404)
            />
          ) : (
        ...
        <View style={styles.actions}>
          <TouchableOpacity 
            ...
            onPress={() => router.push({ 
              ...
              // On passe finalAvatarUrl décodé vers la page de profil
              params: { avatar: finalAvatarUrl ? encodeURIComponent(finalAvatarUrl) : '' } 
            })}
          >
