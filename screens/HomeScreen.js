...
export default function HomeScreen() {
  ...
  const handleLendGame = async () => {
    if (!loanedToName.trim()) {
      Alert.alert("Erreur", "Veuillez indiquer le nom de la personne.");
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        ...
        setLoanedToName('');
        setGameToLend(null);
        Alert.alert("Succès", `Jeu prêté à ${loanedToName.trim()} !`);
        fetchGames(); // On rafraîchit la liste
      } else {
        Alert.alert("Erreur", data.message);
        setIsLoading(false);
      }
    } catch (error) {
      Alert.alert("Erreur", "Impossible de prêter le jeu.");
      setIsLoading(false);
    }
  };
      ...
      >
        ...
        <View style={styles.cardInfo}>
          ...
          <View style={styles.statusRow}>
            ...
            {/* NOUVEAU : Bouton de prêt rapide (Seulement sur Physique & si pas déjà prêté) */}
      ...
      <Modal visible={lendModalVisible} transparent animationType="fade">
        <View style={styles.lendModalOverlay}>
          <View style={styles.lendModalContent}>
            <Text style={styles.lendModalTitle}>Prêter ce jeu</Text>
            <Text style={styles.lendModalText}>
              À qui prêtez-vous <Text style={{fontWeight: 'bold', color: '#fff'}}>{gameToLend?.title}</Text> ?
            </Text>
            ...
            <View style={styles.lendModalActions}>
              <TouchableOpacity 
                style={[styles.lendModalBtn, styles.lendModalBtnCancel]} 
                onPress={() => setLendModalVisible(false)}
              >
                <Text style={styles.lendModalBtnTextCancel}>Annuler</Text>
              </TouchableOpacity>
              <TouchableOpacity 
                style={[styles.lendModalBtn, styles.lendModalBtnConfirm]} 
                onPress={handleLendGame}
              >
                <Text style={styles.lendModalBtnTextConfirm}>Confirmer</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
