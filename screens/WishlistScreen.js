...
export default function WishlistScreen() {
  ...
  const promptAcquire = (game) => {
    Alert.alert(
      ...
      [
        { text: i18n.t('wishlist.acquire_yes'), onPress: () => acquireGame(game, game.format || activeFormat) },
        { text: i18n.t('common.cancel'), style: "cancel" },
      ]
    );
  };
  ...
  const applyFiltersAndSort = () => {
    let result = games.filter(g => (g.format || 'physical') === activeFormat);
  ...
  const platformSet = new Set(games.filter(g => (g.format || 'physical') === activeFormat).map(g => getStandardPlatform(g.platform)));
  ...
  return (
    <View style={styles.container}>
      ...
      {/* SÉLECTEUR DE FORMAT : Structure et icônes synchronisées avec HomeScreen */}
      {!isSelectionMode && (
      ...
      {displayedGames.length === 0 ? (
        <View style={styles.emptyContainer}>
          ...
          <TouchableOpacity 
            ...
            onPress={() => router.push({ pathname: '/search', params: { defaultStatus: 'wishlist', defaultFormat: activeFormat }})}
          >
      ...
      {isSelectionMode ? (
        ...
        <TouchableOpacity style={styles.fab} onPress={() => router.push({ pathname: '/search', params: { defaultStatus: 'wishlist', defaultFormat: activeFormat }})}>
...
const styles = StyleSheet.create({
  ...
  // STYLES SYNCHRONISÉS : Strictement identiques à HomeScreen.js
  toggleContainer: { flexDirection: 'row', backgroundColor: '#202020', borderRadius: 50, marginHorizontal: 20, marginBottom: 16, marginTop: 8, padding: 4 },
